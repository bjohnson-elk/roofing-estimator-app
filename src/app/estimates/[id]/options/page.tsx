...
function getUpgradeItem(selection: UpgradeSelection) {
  const item = selection.upgrade_items;
...
function getUiUpgradeName(selection: UpgradeSelection) {
  const item = getUpgradeItem(selection);
  const name = item?.upgrade_name ?? "";

  if (/system plus/i.test(name)) return "System Plus";
  if (/silver pledge/i.test(name)) return "Silver Pledge";
  if (/snowcountry|snow country|upgraded ridge/i.test(name)) {
    return "Upgraded Ridge Vent";
  }
  if (/ridge runner|standard ridge/i.test(name)) {
    return "Standard Ridge Vent";
  }
...
export default function EstimateOptionsPage() {
  ...
  async function loadPage() {
    ...
    if (optionIds.length > 0) {
      const { data: upgradeData, error: upgradeError } = await supabase
        .from("estimate_option_upgrades")
        .select("id, estimate_option_id, active, upgrade_items(upgrade_name, upgrade_category)")
  ...
  const activeUpgradeSet = useMemo(() => {
    return new Set(
      upgradeSelections
        ...
        .map((selection) => {
          const uiName = getUiUpgradeName(selection);
          return uiName ? `${selection.estimate_option_id}-${uiName}` : "";
        })
        .filter(Boolean)
    );
  }, [upgradeSelections]);
  ...
  async function applyUpgrade(optionId: string, upgradeName: string) {
    ...
    const { error } = await supabase.rpc("toggle_estimate_option_upgrade_ui", {
    ...
    const wasActive = activeUpgradeSet.has(`${optionId}-${upgradeName}`);

    setSuccessMessage(
      wasActive
        ? `${upgradeName} removed. Pricing refreshed.`
        : `${upgradeName} applied. Pricing refreshed.`
    );
